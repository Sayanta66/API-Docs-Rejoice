...
<div role="navigation" aria-label="{{ _('Page navigation') }}">
  <ul class="wy-breadcrumbs">
    {%- block breadcrumbs %}
      ...
      <li><a href="{{ pathto(master_doc) }}" class="ks-breadcrumbs icon icon-home">{{ project }} </a></li>
      
      {%- if version and theme_breadcrumb_version %} 
      <li><a href="{{ pathto(master_doc) }}" class="ks-breadcrumbs icon icon-home">{{ version }} </a> </li>
      {% endif %} 
      
      {%- for doc in parents %}
        <li><a href="{{ doc.link|e }}" class= "ks-breadcrumbs">{{ doc.title }}</a></li>
      {%- endfor %}
      
      <li>{{ title }}</li>
    {%- endblock %}
    
    {# Disabling View page source option and links etc. - start
    ...
    {%- endblock %}
   {# Disabling View page source option - end #}
  </ul>
  {# Disabling sidebar previous next buttons as we need them at bottom of the page only #}
  {#
